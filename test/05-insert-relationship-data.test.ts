...
const insertMovieGenres = (
  ...
  return (
    ...
    genres.map(genre =>`('${movieId}','${genreRows.find(it =>it.genre === genre)!.id}')`)
  );
};
...
const insertMovieActors = (
  ...
  return (
    ...
    actors.map(actor =>`('${movieId}','${actorRows.find(it =>it.full_name === actor)!.id}')`)
  );
};
...
const insertMovieDirectors = (
  ...
  return (
    ...
    directors.map(director =>`('${movieId}','${directorRows.find(it =>it.full_name === director)!.id}')`)
  );
};
...
const insertMovieKeywords = (
  ...
  return (
    ...
    keywords.map(keyword =>`('${movieId}','${keywordRows.find(it =>it.keyword === keyword)!.id}')`)
  );
};
...
const insertMovieProductionCompanies = (
  ...
  return (
    ...
    productionCompanies.map(productionCompanie =>`('${movieId}','${productionCompanyRows.find(it =>it.company_name === productionCompanie)!.id}')`)
  );
};
...
describe("Insert Relationship Data", () => {
  ...
  it(
    ...
    async done => {
      const movies = await CsvLoader.movies();
      const directorRows = (await db.selectMultipleRows(`SELECT id,full_name from directors`)) as DirectorRow[];
  ...
  it(
    ...
    async done => {
      const movies = await CsvLoader.movies();
      const productionCompanyRows = (await db.selectMultipleRows(`SELECT id,company_name from production_companies`)) as ProductionCompanyRow[];
